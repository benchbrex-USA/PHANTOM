...
impl TemplateVars {
    ...
    pub fn env_vars(&self) -> HashMap<String, String> {
        ...
        vars.insert(
            "AGENT_CAN_DELEGATE".into(),
            self.can_delegate.to_string(),
        );
...
impl ClaudeMdGenerator {
    ...
    pub fn render(
        ...
        // Header
        writeln!(out, "# CLAUDE.md — {} Instructions", vars.agent_display_name).unwrap();
        writeln!(out).unwrap();
        ...
        writeln!(
            out,
            "Generated at: {{timestamp will be set at spawn time}}"
        )
        .unwrap();
        writeln!(out, "Zero-footprint: this file will be deleted after agent completion.").unwrap();
    ...
    fn render_knowledge(
        ...
        if relevant.is_empty() {
            writeln!(
                out,
                "*No chunks matched this agent's knowledge scope.*\n"
            )
            .unwrap();
            return;
        }
...
pub fn cleanup_all(files: &[GeneratedClaudeMd], base_dir: &Path) {
    ...
    if base_dir.exists() {
        if base_dir.read_dir().map(|mut d| d.next().is_none()).unwrap_or(false) {
...
mod tests {
    ...
    fn test_all_roles_render_successfully() {
        ...
        for role in crate::ALL_ROLES {
            ...
            let content = gen.render(&vars, &[]).unwrap();
            assert!(content.contains(&format!("# CLAUDE.md — {} Instructions", role.display_name())));
        }
    }
